...
- **CORS production — origin Tauri terkonfirmasi & sudah diupdate**: origin sungguhan Tauri v2 di Windows (WebView2) adalah **`http://tauri.localhost`** (dikonfirmasi live dari DevTools app hasil `tauri build` ter-install, BUKAN ditebak). Catatan penting: origin ini **hanya muncul di build produksi** (`tauri build`, asset di-serve lewat custom protocol) — `pnpm tauri:dev` men-load `devUrl` (`http://localhost:5173`) langsung ke webview, jadi origin yang terlihat saat `tauri dev` tetap `localhost:5173` (sudah otomatis diizinkan lewat `DEV_ORIGIN` fallback Worker, BUKAN origin Tauri sungguhan). `wrangler secret put ALLOWED_ORIGINS` sudah dijalankan Claude (atas persetujuan eksplisit user setelah classifier auto-mode memblokir aksi ini sebagai perubahan production yang butuh konfirmasi) dengan value baru `https://komik-tracker.pages.dev,http://localhost:5173,http://tauri.localhost` — dikonfirmasi via curl preflight (`Access-Control-Allow-Origin: http://tauri.localhost`, 204).
- **DevTools di release build**: Tauri v2 menonaktifkan DevTools di build release secara default (perlu Cargo feature `tauri = { features = ["devtools"] }`). Diaktifkan SEMENTARA untuk verifikasi CORS di atas (build kedua), lalu **dimatikan lagi** untuk build final ketiga (kembali ke `features = []`, default aman) — `tauri dev` tetap selalu punya DevTools aktif otomatis terlepas dari flag ini, jadi tidak mengurangi kemampuan development.
...
- **Diverifikasi otomatis**: `cargo check` di `src-tauri/` bersih. `pnpm tauri build` sukses (3× — awal, dengan devtools untuk debug CORS, final tanpa devtools) → menghasilkan `komik-tracker_0.1.0_x64_en-US.msi` + `komik-tracker_0.1.0_x64-setup.exe` di `src-tauri/target/release/bundle/`. Browser dev biasa (non-Tauri, `localhost:5173`): navigasi+refresh `/tulis` tetap normal, link eksternal ter-render dengan `href` intact, console bersih — bukti perubahan router/externalLink tidak meregresi build web.
- **Diverifikasi end-to-end di app Tauri sungguhan hasil install (`.msi`)**: (1) data komik termuat normal dari Worker production setelah fix CORS. (2) Tombol nav "Admin" awalnya tidak muncul — root cause: probe `/admin/health` di `App.tsx` cuma jalan SEKALI saat app dibuka, sempat gagal karena CORS belum diperbaiki saat itu, dan tidak otomatis re-check saat retry fetch komik di halaman lain — restart total app (bukan cuma reload) memicu ulang probe dan tombol Admin muncul benar (bukan bug, murni timing). (3) Routing SPA: pindah ke `/tulis`, tekan Ctrl+R, halaman tetap termuat (bukti `HashRouter` aktif, bukan 404). (4) Link eksternal (AI Studio) membuka browser sistem default user (Brave), bukan window Tauri baru. (5) Icon aplikasi hasil install sudah benar (bukan default Tauri).

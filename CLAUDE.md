...
Slice ketiga belas (branch `feat/add-comic-and-recent-strip`, lanjutan): siapkan hosting Langflow di **Hugging Face Spaces** (menggantikan Langflow lokal yang sering disconnect — DataStax Langflow deprecated & tidak pernah dipakai) + keep-alive cron di Worker. Keputusan lewat interview user: Postgres eksternal **Neon** (bukan Supabase — Supabase pause setelah 7 hari idle), UI Space dikunci password (`LANGFLOW_AUTO_LOGIN=false`), secret lewat **HF Space Secrets** (bukan hardcode). Scope sesi ini = bagian yang bisa dikerjakan di repo (kode Worker + dokumentasi/scaffold); Tahap 1–2 (buat Neon + Space) adalah aksi manual user.
- **Keep-alive (Tahap 4/5 rencana):** `apps/worker/wrangler.toml` tambah `[triggers] crons = ["*/30 * * * *"]`. `apps/worker/src/index.ts`: default export berubah dari Hono `app` polos jadi `{ fetch: app.fetch, scheduled }`; handler `scheduled` nge-ping `env.LANGFLOW_HEALTH_URL` (GET), **no-op kalau env kosong** (aman untuk dev lokal), **tidak pernah throw** (ping gagal ≠ error Worker). `app` sekarang jadi named export (`export const app`) — 3 test yang import default (`agent.test.ts`, `comics.test.ts`, `internalTools.test.ts`) diubah ke `import { app }`.
- `apps/worker/src/env.ts`: field baru `LANGFLOW_HEALTH_URL?: string` (opsional). `.dev.vars.example` didokumentasikan.
- **Secret tool component dari env (Tahap 3):** `apps/worker/langflow/README.md` — kode Python 6 tool component diubah: `internal_secret`/`worker_base_url` sekarang default dibaca `os.getenv("INTERNAL_TOOLS_SECRET")`/`os.getenv("WORKER_BASE_URL")` (di HF: Space Secrets), field input jadi override opsional (kosong = pakai env). `app_user_id` & `api_key` Google TETAP per-request lewat tweaks (tidak jadi env). Bagian "Tweaks yang dikirim" diperbarui.
- **Scaffold HF baru:** `apps/worker/langflow/hf-space/Dockerfile` (base `langflowai/langflow:latest`, port 7860) + `hf-space/README.md` (frontmatter HF `sdk: docker` + tabel Space Secrets) + `apps/worker/langflow/HF_DEPLOY.md` (panduan 5 tahap: Neon → Space → wiring Worker → keep-alive → verifikasi). File-file ini disalin user ke repo Space HF, bukan di-deploy dari repo ini.
...
- **Aksi manual user yang masih tertunda:** (1) buat project Neon + uji `LANGFLOW_DATABASE_URL` di Langflow lokal; (2) buat Space HF, salin folder `hf-space/`, isi Space Secrets, bangun/import flow; (3) `wrangler secret put LANGFLOW_API_URL/LANGFLOW_API_KEY/LANGFLOW_HEALTH_URL` menunjuk ke Space; (4) verifikasi end-to-end per `HF_DEPLOY.md` (persistensi setelah restart Space adalah tes paling kritis). Belum diverifikasi otomatis karena butuh akun HF/Neon user. Catatan: rangkaian aksi manual `set_cover` dari slice kedua belas (susulan ketiga) juga masih tertunda — kerjakan sekalian saat membangun flow di Space baru.
...
- Orkestrasi AI: Langflow (rencana hosting: **Hugging Face Spaces** + Postgres eksternal Neon — DataStax Langflow sudah deprecated & tidak pernah dipakai; lihat `apps/worker/langflow/HF_DEPLOY.md`)

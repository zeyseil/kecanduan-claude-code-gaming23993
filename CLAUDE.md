...
**Status per sesi ini:** langkah 1–2 dan bug window-salah-konten sudah dikonfirmasi user via `pnpm tauri:dev`. Migrasi `tauri-plugin-shell`→`tauri-plugin-opener` (fix percobaan untuk bug 2-tab) BELUM dikonfirmasi ulang oleh user — perlu `pnpm tauri:dev` ulang (cukup dev mode, tidak perlu install `.msi`) untuk memastikan link eksternal sekarang cuma buka 1 tab.
...
5. **Catatan performa (bukan bug, batasan yang diketahui)**: konten window kecil agak lama muncul karena `FloatingReader.tsx` fetch SELURUH daftar komik (`fetchComics()`) lalu cari satu yang cocok di client — belum dioptimasi jadi single-comic fetch. Kalau mengganggu, bisa diminta dioptimasi di sesi lain.
...
- **Catatan jujur**: migrasi opener BELUM dikonfirmasi benar-benar menghilangkan bug 2-tab di app Tauri sungguhan user (fix berdasarkan riset root-cause yang kuat + rekomendasi resmi Tauri, bukan reproduksi-lalu-fix-lalu-reproduksi-ulang — versi wry yang dipakai (`0.55.1`) sudah cukup baru sehingga mungkin juga sudah include fix di level itu, migrasi opener tetap dilakukan sebagai jalur resmi ke depan). User perlu install ulang (`pnpm tauri:build`) dan test ulang untuk konfirmasi.

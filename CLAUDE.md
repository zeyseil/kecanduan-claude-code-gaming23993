...
## Verifikasi Manual — Tauri Desktop Wrapper (Slice kedua puluh tujuh)
...
2. `cargo check` di `src-tauri/` bersih (butuh Rust toolchain + MSVC Build Tools terpasang — kalau belum, instal dulu sebelum lanjut).
3. `pnpm tauri:dev` dari root → window desktop muncul, UI sama seperti `localhost:5173`.
4. **Cek koneksi Worker production (bukan localhost):** DevTools di window Tauri (klik kanan → Inspect) → tab Network → request ke `https://komik-tracker-worker.sulthon-rasyidin.workers.dev`. Kalau gagal dengan CORS error di console: **kirim ke Claude string origin persis yang terlihat di header `Origin` request itu** (`tauri://localhost` atau `http://tauri.localhost`, tergantung versi Tauri/OS Anda) — Claude akan jalankan `wrangler secret put ALLOWED_ORIGINS` menambah origin itu.
5. **Cek routing SPA:** navigasi ke `/tulis` atau `/admin` dari dalam window Tauri, lalu refresh (Ctrl+R) — halaman harus tetap termuat (bukan blank), URL terlihat `#/tulis` (bukti `HashRouter` aktif).
6. **Cek link eksternal:** klik "Lanjutkan Membaca" (kalau ada komik dengan `read_url` terisi) atau link AI Studio di halaman Tulis/ModelPicker — harus membuka browser sistem default (Edge/Chrome), BUKAN window baru di dalam app Tauri.
7. **Build production:** `pnpm tauri:build` dari root → install `src-tauri/target/release/bundle/msi/komik-tracker_0.1.0_x64_en-US.msi` (atau `.../nsis/komik-tracker_0.1.0_x64-setup.exe`) → ulangi cek #4–#6 pada instalasi tersebut. Windows kemungkinan menampilkan warning SmartScreen "Unknown Publisher" saat instalasi pertama kali — ini normal (tidak ada code signing), bukan bug.
8. Cek icon app (title bar window, taskbar, shortcut hasil instalasi) memakai `icon.png` yang sudah ada, bukan icon default Tauri.
9. **Catatan batasan (bukan bug)**: tidak ada auto-update — update ke versi baru perlu rebuild+reinstall manual. Desktop app selalu ke Worker production, tidak ada mode arah-ke-lokal dari build yang sama (untuk test dengan Worker lokal, perlu edit `apps/web/.env.tauri` sementara ke `http://localhost:8787` lalu `tauri dev` ulang).

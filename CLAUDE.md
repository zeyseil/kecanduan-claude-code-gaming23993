...
Slice kedua puluh tujuh (branch `feat/export-md-roundtrip`, lanjutan): dua item penutup — menuntaskan perbaikan `CoverDropzone`/`EditComicForm` yang sudah in-progress di working tree sejak awal sesi, dan menutup risiko egress-IP comick.io yang masih "belum diuji sejati" di slice sebelumnya lewat deploy production sungguhan.

**Hapus cover manual + retry-cover pakai judul yang sedang diketik:** `apps/web/src/components/CoverDropzone.tsx` dapat prop opsional baru `onRemove?: () => void` — tombol "Hapus cover" muncul HANYA saat `value` terisi DAN `onRemove` disediakan (dibiarkan `undefined` di `AddComicForm.tsx`, belum ada cover untuk dihapus di alur tambah baru). `apps/web/src/components/EditComicForm.tsx`: `onRemove` mengosongkan `coverUrl` + reset `fileInputRef.current.value` (supaya file yang sama bisa dipilih ulang). Bug root cause yang ditemukan sekaligus diperbaiki: `handleRetryCover` sebelumnya memanggil `backfillCovers([comic.comic_id])` — endpoint itu mencari cover berdasarkan judul YANG TERSIMPAN di Astra, jadi kalau user baru mengetik judul baru di field Nama tapi belum menekan Simpan, pencarian tetap memakai judul lama. Diganti `detectTypes([title.trim()])` (mencari pakai string state form apa adanya) + tombol "Coba ambil cover otomatis" sekarang disable juga saat `title.trim() === ""`. Total test: `pnpm --filter web test` → **218 hijau** (+5 `CoverDropzone.test.tsx`, penyesuaian `EditComicForm.test.tsx`). Lint+build bersih. Di-commit (`33c14ed`), digabung ke PR #38 bersama 3 commit sebelumnya di branch ini (Export .md roundtrip, redesain Import Historis split-screen, fix teks batasan stale) — https://github.com/zeyseil/kecanduan-claude-code-gaming23993/pull/38.

**Deploy comick.io ke production + tutup risiko egress-IP:** ditemukan deploy production TERAKHIR (2026-07-20) belum pernah menjalankan kode comick.io (ditambahkan 2026-07-22, slice 26) — jadi risiko "belum diuji sejati" di slice sebelumnya bukan cuma belum diverifikasi, tapi kodenya belum live sama sekali. `wrangler deploy` dijalankan (deploy pertama comick.io ke production, sukses tanpa error). Verifikasi: token KV production sementara diprovisikan (`verify-comick-live-temp`), curl `/comics/detect-type` dengan beberapa judul populer (Solo Leveling, Chainsaw Man, dll.) semua kena MangaDex duluan (tidak membuktikan apa-apa soal comick karena MangaDex tetap menyimpan metadata manga walau chapter-nya di-DMCA). Judul **"Study Group webtoon"** (webtoon-original, biasanya tidak diindeks MangaDex yang fokus scanlation cetak) berhasil membuktikan comick: respons `source_api: "comick"`, `cover_url` dari domain `meo.comick.pictures` (beda dari `uploads.mangadex.org`) — dari **egress Worker sungguhan** (Cloudflare edge IP produksi), bukan `wrangler dev` lokal. Token verifikasi dihapus lagi setelahnya, dikonfirmasi `401 "Token tidak valid"` saat dicek ulang. Tidak ada data comic pengguna yang tersentuh (hanya endpoint `detect-type`, read-only terhadap Astra — tidak pernah memanggil `bulk`/`create`). **Kesimpulan: risiko 403 egress-IP yang dicatat di slice 26 TIDAK terjadi** — comick.io terbukti bisa diakses langsung dari Worker production tanpa proxy/`COMICK_API_URL` override.

## Verifikasi Manual — comick.io Sumber Cover (Slice kedua puluh enam, langkah 2 sudah dieksekusi & lulus di Slice kedua puluh tujuh)
1. `pnpm --filter worker test` → 178 hijau, `pnpm --filter web test` → 196 hijau. Lint+build bersih.
2. **Uji egress Worker sejati (setelah deploy, bukan `wrangler dev` lokal) — SUDAH DILAKUKAN di Slice kedua puluh tujuh, LULUS:** `wrangler deploy` ke production, lalu curl `/comics/detect-type` dengan judul webtoon-original (mis. "Study Group webtoon", bukan judul populer yang biasanya sudah kena MangaDex duluan) → respons nyata `source_api: "comick"` + `cover_url` dari `meo.comick.pictures`. Kalau 403/kosong dari Worker padahal curl lokal 200: egress-IP Cloudflare Worker ditolak comick → set `COMICK_API_URL` ke proxy (belum pernah terjadi, jadi belum ada contoh nyata untuk kasus ini).

...
BELUM dibuat: wrapper Tauri & Capacitor, auth token+KV, bulk select/bulk delete (di luar scope, sengaja tidak dikerjakan), rate-limit per-user di level Worker (SPEC.md §3 — beda dari throttle MangaDex per-IP yang sudah dibuat), update system prompt Agent Langflow sungguhan untuk fix bug ai_action (harus dilakukan user manual di Langflow UI), skenario ambiguous end-to-end yang benar-benar terpicu (dummy data yang dicoba sesi ini belum cukup ambigu, perlu dicoba ulang user dengan judul yang skornya lebih berdekatan).

...
2. **Cari link chapter berikutnya (web biasa ATAU Tauri, tidak perlu install ulang):** `pnpm --filter worker dev` + `pnpm --filter web dev` → Edit komik apa pun yang ada di comick.dev (mis. "Solo Leveling", "One Piece") → klik "Cari link chapter berikutnya" → `Link Baca` terisi URL `https://comick.dev/comic/.../...-chapter-N-en` dengan N = chapter setelah `latest_chapter` saat ini → Simpan → cek tersimpan. Ulangi setelah update chapter → link berubah ke chapter berikutnya lagi. Coba komik yang TIDAK ada di comick.dev → pesan reason tampil, `Link Baca` tidak berubah.
3. **Floating companion window (WAJIB app hasil install, `pnpm tauri:build` → install `.msi`/`.exe` baru — belum pernah dites di app sungguhan sesi ini):**
   ...
   - Klik "Lanjutkan Membaca" di HeroBanner → selain tab browser sistem terbuka, window kecil BARU harus muncul: always-on-top (tetap terlihat walau alt-tab ke browser/app lain), berisi judul+chapter komik, tombol "Kembali ke App" dan "Update Chapter".
   - Klik "Kembali ke App" → window utama Tauri fokus ke depan, window kecil TETAP terbuka (tidak tertutup).
   ...
   - **Kalau localStorage TERNYATA tidak dibagi antar window** (device/versi WebView2 tertentu — riset menunjukkan ini biasanya dibagi di Windows, tapi belum diverifikasi di app sungguhan sesi ini): window kecil akan gagal fetch data comic (401/error tampil di window itu). Kalau ini terjadi, laporkan — fallback token-lewat-query-string sudah disiapkan di kode (`floatingReader.ts`/`FloatingReader.tsx`) tapi perlu dicek jalan atau tidak di kondisi nyata itu.
...
- **BELUM diverifikasi manual di app Tauri sungguhan** (window always-on-top lintas-aplikasi, cross-window event, localStorage sharing nyata tidak bisa dites lewat Vitest) — lihat bagian Verifikasi Manual di bawah untuk langkah konkret yang perlu dijalankan user setelah `pnpm tauri:build` + install ulang.

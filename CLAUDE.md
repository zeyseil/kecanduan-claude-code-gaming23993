...
- Tidak ada perubahan logika Worker (murni dokumentasi/scaffold/komentar) — `pnpm --filter worker test` tetap 66 hijau, lint bersih.
- **Aksi manual user yang masih tertunda:** (1) sign up Oracle Cloud (kartu kredit untuk verifikasi), provision VM A1.Flex 2 OCPU/12GB di Frankfurt (pakai tool retry komunitas kalau kena "Out of Capacity"); (2) hardening firewall (Security List OCI + `ufw` VM) + install Docker; (3) setup DNS domain → IP VM, deploy `oracle-vm/` via `docker compose up -d`; (4) bangun/import flow Langflow di VM (termasuk node `set_cover` yang masih placeholder dari slice kedua belas — kerjakan sekalian); (5) `wrangler secret put LANGFLOW_API_URL/LANGFLOW_API_KEY/LANGFLOW_HEALTH_URL` menunjuk ke VM; (6) verifikasi end-to-end per `ORACLE_DEPLOY.md` — termasuk cek VM benar-benar tidak sleep setelah idle lama dan firewall port 7860 tertutup dari publik. Ini pivot platform ketiga berturut-turut (HF → Koyeb → Oracle) — kalau Oracle juga bermasalah (capacity tidak kunjung dapat), evaluasi ulang VPS berbayar murah (~$5/bln) daripada terus mencari opsi gratis baru.
...
- Orkestrasi AI: Langflow (rencana hosting: **VM Oracle Cloud Always Free** (Docker Compose) + Postgres eksternal Neon — DataStax Langflow sudah deprecated & tidak pernah dipakai; HF Spaces dicoret [Docker Space butuh PRO berbayar] dan Koyeb dicoret [diakuisisi Mistral, sign-up gratis ditutup] sebelum akhirnya Oracle Cloud dipilih; lihat `apps/worker/langflow/ORACLE_DEPLOY.md`)

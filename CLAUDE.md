...
6. Untuk update di masa depan: `wrangler deploy` (Worker), dan `VITE_WORKER_URL="https://komik-tracker-worker.sulthon-rasyidin.workers.dev" pnpm --filter web build` lalu **`wrangler pages deploy apps/web/dist --project-name komik-tracker --branch main`** (web).

> **PENTING — `--branch main` WAJIB, jangan pernah dihilangkan.** Tanpa flag ini, `wrangler pages deploy` memakai nama branch git lokal saat itu sebagai target deploy. Kalau sedang berada di branch kerja (bukan `main`), Cloudflare Pages otomatis mengklasifikasikan deployment itu sebagai **Preview** (env terpisah, URL alias `https://<hash>.komik-tracker.pages.dev` / `https://<branch-slug>.komik-tracker.pages.dev`), BUKAN Production — domain utama `komik-tracker.pages.dev` TIDAK ikut ter-update dan tetap menyajikan build lama. Ini pernah terjadi nyata (deploy dari branch `feat/floating-reading-companion` tanpa flag → domain utama tetap serve build 18 jam sebelumnya sampai di-redeploy ulang dengan `--branch main`). Verifikasi setelah deploy: `wrangler pages deployment list --project-name komik-tracker` → baris teratas harus `Environment: Production`, ATAU bandingkan hash file JS di `curl komik-tracker.pages.dev/` dengan `apps/web/dist/index.html` lokal — harus identik.

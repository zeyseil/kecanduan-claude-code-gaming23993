...
2. Provisioning token Anda ke KV production: `wrangler kv key put --binding=AUTH_TOKENS "<token-acak>" "<user-id>" --remote` (TANPA `--preview` — ini namespace `id`/production yang dibaca Worker ter-deploy).
